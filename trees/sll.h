...
*
*   changelog :- 11/30/2021 
            -action:    added getHead() and getTail()
                        reworked getNode()
            -reason:    ease of use for GeneralTree.BFS()
*/
 ...
 * 
 *  constraint: if index > SLL->size, returns tail
 * 
 *  Params:     int index
 */
template <typename T>
...
T SLL<T>::popBack(){
    ...
    if ( this->getSize() > 1 ) {
        this->setTail( this->getNode( this->getSize()-2));
...
*
*   27 - 11 - 2021 :: Modified to treat items as Nodes and performs getData() on each ListNode.
*
*   Params:     None
*/
template <typename T>
void SLL<T>::printList(){
    if ( this->isEmpty() ) {
        ...
        for ( int i=1; i < this->getSize(); i++ ) {
            listElements << currentNode->getData()->getData() << ", ";
            currentNode = currentNode->getNext();
        }
        listElements << currentNode->getData()->getData() << " ]";
